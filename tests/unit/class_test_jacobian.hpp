...
class test_split_dense_Jacobian{
 public:
   test_split_dense_Jacobian(std::size_t domain_dimension, std::size_t codomain_dimension,
       std::size_t number_of_edges, std::size_t function_cost){

    Matrix_free_information
      jacobian_information{domain_dimension, codomain_dimension, number_of_edges};
    
    Split_dense_Jacobian split_dense_data{jacobian_information, function_cost};

    if(split_dense_data.function_cost() != function_cost){
     test_function_cost_value = false; 
     print_test_state();
    }
    else{test_function_cost_value = true;}

   }

   void print_test_state() const{
    
     std::cout<< "State test_split_dense_Jacobian:\n";

     std::cout<< "test_function_cost_value:";
     if(test_function_cost_value){
      std::cout << "successful.\n";
     }
     else{std::cout << "failed.\n";}
     std::cout << "\n";
  ...
  static const std::string format_file_name(const std::string& file_name){
    if(file_name == "case_0" || file_name == "case_1" || file_name == "case_2" ||
      ...
      std::string file_path =
        std::string(FIXTURE_DIR) + "/sparse_jacobian_sample_tests/" + file_name;
      return file_path;
    }
    return file_name;
  }
...
class test_split_sparse_Jacobian{
 public:
  test_split_sparse_Jacobian(const std::string& file_path, std::size_t function_cost){

    {
      Sparse_Jacobian jacobian = Sparse_Jacobian::from_file(file_path+ "_sparse_data");

      Split_sparse_Jacobian split_sparse_data{jacobian, function_cost};

      if(split_sparse_data.function_cost() != function_cost){
        test_function_cost_value = false;
      }
      else{test_function_cost_value = true;}
    }
    ...
    {
      std::size_t function_cost_lhs = 125, function_cost_rhs = 212;

      Sparse_Jacobian lhs = 
        Sparse_Jacobian::from_file(
            std::string(FIXTURE_DIR) + "/sparse_jacobian_sample_tests/case_1_MxM_lhs_sparse_data");

      Split_sparse_Jacobian split_sparse_lhs{lhs, function_cost_lhs};
      
      Sparse_Jacobian rhs =
        Sparse_Jacobian::from_file(
            std::string(FIXTURE_DIR) + "/sparse_jacobian_sample_tests/case_1_MxM_rhs_sparse_data");

      Split_sparse_Jacobian split_sparse_rhs{rhs, function_cost_rhs};

      Split_sparse_Jacobian split_sparse_product = split_sparse_lhs * split_sparse_rhs;

      if(split_sparse_product.function_cost() != function_cost_lhs + function_cost_rhs){

        test_multiplication_function_cost = false;
      }
      else{test_multiplication_function_cost = true;}
    }

    if(test_function_cost_value && test_multiplication_function_cost){
  ...
  void print_test_state() const{
   ...
   std::cout<< "State test_split_sparse_Jacobian:\n";
   ...
   std::cout << "test_multiplication_function_cost:";
   if(test_multiplication_function_cost){
    std::cout << "successful.\n";
   }
   else{std::cout << "failed.\n";}
   std::cout << "\n";
 ...
 protected:
  bool test_function_cost_value;
  bool test_multiplication_function_cost;
  bool were_all_test_successful;
};
