...
std::string test_chain<Jacobian_T, Basic_info_T>::case_to_path_with_data(
  ...
  if(case_name == "case_0"){
    if constexpr (std::is_same_v<Basic_info_T, Jacobian_information>){
      return std::string(FIXTURE_DIR) + "/chain_test_cases/case_0_jacobian_information";
    }

    else{
      return std::string(FIXTURE_DIR) + "/chain_test_cases/case_0_matrix_free";
    }
  }

  if(case_name == "case_1"){
    if constexpr (std::is_same_v<Basic_info_T, Jacobian_information>){
      return std::string(FIXTURE_DIR) + "/chain_test_cases/case_1_jacobian_information"; 
    }

    else{
      return std::string(FIXTURE_DIR) + "/chain_test_cases/case_1_matrix_free"; 
    }
  }
...
std::string test_chain<Jacobian_T, Basic_info_T>::case_to_validate_path(
  ...
  if(case_name == "case_0"){
    if constexpr (std::is_same_v<Basic_info_T, Jacobian_information>){
      return std::string(FIXTURE_DIR) + "/chain_test_cases/validate/case_0_jacobian_information"; 
    }

    else{
      return std::string(FIXTURE_DIR) + "/chain_test_cases/validate/case_0_matrix_free"; 
    }
  }
...
std::string test_chain<Jacobian_T, Basic_info_T>::case_to_validate_path_generator(){
  ...
  if constexpr(std::is_same_v<Basic_info_T, Jacobian_information>){
    return std::string(FIXTURE_DIR) + "/generator_chain_cases/validate/jacobian_information";
  }

  else{ 
    return std::string(FIXTURE_DIR) + "/generator_chain_cases/validate/matrix_free_information";
  }
...
std::string test_chain<Sparse_Jacobian, Matrix_free_sparse_information>::
  ...
  if(case_name == "case_0"){
    return std::string(FIXTURE_DIR) + "/chain_test_cases/case_0_sparse";
  }

  else if(case_name == "case_1"){
    return std::string(FIXTURE_DIR) + "/chain_test_cases/case_1_sparse";
  }
...
std::string test_chain<Sparse_Jacobian, Matrix_free_sparse_information>::
  ...
  if(validate_name == "case_0"){
    return std::string(FIXTURE_DIR) + "/chain_test_cases/validate/sparse/case_0_"+
      std::to_string(jacobian_index);
  }

  else if(validate_name == "case_1"){
    return std::string(FIXTURE_DIR) + "/chain_test_cases/validate/sparse/case_1_"+
      std::to_string(jacobian_index);
  }
...
std::string test_chain<Sparse_Jacobian, Matrix_free_sparse_information>::
  ...
  return std::string(FIXTURE_DIR) + "/generator_chain_cases/validate/sparse_" +
    std::to_string(jacobian_index);
}
...
template<>
class test_chain<Split_dense_Jacobian, Split_reversal_dense_information>{
 public:
  test_chain(const std::string& case_name){

    if(!(case_name == "case_0" || case_name == "case_1")){
      
      throw std::invalid_argument("Error encountered while creating test_chain split reversal "
          "object. case_name takes as possible values either case_0 or case_1");
    }

    const std::string functions_cost_path = 
      std::string(FIXTURE_DIR) + "/chain_test_cases/" + case_name + "_functions_cost";
    //Testing chain constructed from generator data.
    {
      std::size_t chain_length = 3, dimension_lb = 4, dimension_ub = 8;
      std::size_t number_edges_lb = 45, number_edges_ub = 88;
      bool is_deterministic = false;
      std::size_t seed = 45;

      Generator<Matrix_free_information> generator{chain_length, dimension_lb, dimension_ub,
          number_edges_lb, number_edges_ub, is_deterministic, seed};

      std::vector<std::size_t> functions_cost;
      functions_cost.reserve(chain_length);
      jacobian_chain<Split_dense_Jacobian, Split_reversal_dense_information>::
        file_to_functions_cost(functions_cost_path, functions_cost);

      std::vector<std::size_t> functions_cost_copy{functions_cost};

      jacobian_chain<Split_dense_Jacobian, Split_reversal_dense_information>
        chain{generator.generate_jacobian_information(), functions_cost};

      if(test_constructor(chain, functions_cost_copy)){
        
        test_chain_from_generator_data = true;
      }
      else{test_chain_from_generator_data = false;}
    }

    //Testing chain constructed from file.
    {
      const std::string file_path =
        std::string(FIXTURE_DIR) + "/chain_test_cases/"+ case_name + "_matrix_free";

      jacobian_chain<Split_dense_Jacobian, Split_reversal_dense_information>
        chain{file_path, functions_cost_path};

      std::vector<std::size_t> functions_cost;
      functions_cost.reserve(chain.size());
      chain.file_to_functions_cost(functions_cost_path, functions_cost);

      if(test_constructor(chain, functions_cost)){

        test_chain_from_files = true;
      }
      else{test_chain_from_files = false;}
    }

    if(test_chain_from_generator_data && test_chain_from_files){
  ...
  void print_test_state(){
    ...
    std::cout << "State of test split dense chain:\n";

    std::cout << "test_chain_from_generator_data: ";
    if(test_chain_from_generator_data){
      
      std::cout << "successful.\n";
    }
    else{std::cout << "failed.\n";}

    std::cout << "test_chain_from_files: ";
    if(test_chain_from_files){
 ...
 protected:
  bool test_chain_from_generator_data;
  bool test_chain_from_files;
  bool were_all_tests_successful;

  bool test_constructor(
      const jacobian_chain<Split_dense_Jacobian, Split_reversal_dense_information>& chain,
      const std::vector<std::size_t>& functions_cost){

    for(std::size_t idx = 0; idx < chain.size(); idx++){
      
      if(chain[idx].function_cost() != functions_cost[idx]){
...
template<>
class test_chain<Split_sparse_Jacobian, Split_reversal_sparse_information>{
 public:
  test_chain(const std::string& case_name){
    ...
    //Testing chain cconstructed from generator data.
    {
      std::size_t chain_length = 3, dimension_lb = 4, dimension_ub = 8;
      std::size_t number_edges_lb = 45, number_edges_ub = 88;
      double density_lb = 0.1, density_ub = 0.4;
      bool is_deterministic = false;
      std::size_t seed = 22;

      Generator<Matrix_free_sparse_information> generator{chain_length, dimension_lb, dimension_ub,
      number_edges_lb, number_edges_ub, density_lb, density_ub, is_deterministic, seed};

      std::vector<std::size_t> functions_cost;
      functions_cost.reserve(chain_length);
      jacobian_chain<Split_sparse_Jacobian, Split_reversal_sparse_information>::
        file_to_functions_cost(functions_cost_path, functions_cost);

      std::vector<std::size_t> functions_cost_copy{functions_cost};

      jacobian_chain<Split_sparse_Jacobian, Split_reversal_sparse_information>
        chain{generator.generate_data(), functions_cost};
    ...
    {
      const std::string file_path = 
        std::string(FIXTURE_DIR) + "/chain_test_cases/"+ case_name + "_sparse";
  ...
  void print_test_state(){
    ...
    std::cout << "State of test split sparse chain:\n";
 ...
 protected:
  ...
  bool test_constructor(
      const jacobian_chain<Split_sparse_Jacobian, Split_reversal_sparse_information>& chain,
      const std::vector<std::size_t>& functions_cost){
