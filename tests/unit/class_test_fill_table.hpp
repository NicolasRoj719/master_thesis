 ...
 public:
  ...
  void run_test_cases(std::size_t memory_limit = std::numeric_limits<std::size_t>::max()){
    ...
    bool test_dense_without_memory = 
      run_test_case_dense(std::string(FIXTURE_DIR) + "/chain_test_cases/case_1_matrix_free");

    bool test_dense_with_memory =
      run_test_case_dense(
          std::string(FIXTURE_DIR) + "/chain_test_cases/case_2_matrix_free", memory_limit);

    bool test_sparse_without_memory = 
      run_test_case_sparse(std::string(FIXTURE_DIR) + "/chain_test_cases/case_1_sparse");

    bool test_sparse_with_memory =
      run_test_case_sparse(
          std::string(FIXTURE_DIR) + "/chain_test_cases/case_2_sparse", memory_limit);
